...
class FileUploadController extends AppController
{
    ...
    public static function generateMultiImagesFieldsText($field, $modelName, $arrModelId, $model, $form)
    {
        $html = "<div style='border:2px solid #000; padding:5px;margin-top:5px;margin-bottom:5px'>";
        $html .= $form->field($model, "{$field}[]")->fileInput(['multiple' => true, 'accept' => 'image/*']);

        if (!empty($model->images->$field)):
$arr  = json_decode($model->images->$field);

            $html .= "<table>";
            foreach ($arr as $key => $item):
                $html .= " <tr style='display: inline;'>";
                $html .= "<td style='padding: 10px'>";
                $html .= "<img src='/images/{$key}' alt='' style='max-width: 200px; max-height: 200px'>";
                $html .= "<br>";
                $html .= "<input placeholder='Подпись[UA]' class=\"img_title\" type=\"text\" value='{$item[0]}'  name=\"img_title[{$key}][ua]\"/>";
                $html .= "<br>";
                $html .= "<input placeholder='Подпись[RU]' class=\"img_title\" type=\"text\" value='{$item[1]}' name=\"img_title[{$key}][ru]\"/>";
                $html .= "<br>";
                $html .= "<a href='" . Url::to(["{$modelName}/delimage", 'img' => base64_encode(implode("|", [$item, $model->id, $arrModelId]))]) . "'>Удалить</a>";
